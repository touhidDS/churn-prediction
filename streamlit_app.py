...
def run_full_pipeline(file_bytes, file_ext, table_name, target_col):
    ...
    if len(num_cols) >= 2:
        corr_s = df_tr.join(y_train).corr()[y_train.name].drop(y_train.name).abs()
        top2 = corr_s.nlargest(2).index.tolist()
        df_tr[f"{top2[0]}_x_{top2[1]}"] = df_tr[top2[0]] * df_tr[top2[1]]
        df_te[f"{top2[0]}_x_{top2[1]}"] = df_te[top2[0]] * df_te[top2[1]]

    # ── Scale ────────────────────────────────────────────────
    scaler = StandardScaler()
    X_tr_sc = pd.DataFrame(scaler.fit_transform(df_tr), columns=df_tr.columns, index=df_tr.index)
    X_te_sc = pd.DataFrame(scaler.transform(df_te),     columns=df_te.columns, index=df_te.index)
    ...
    for i in range(len(corr_m.columns)):
        for j in range(i+1, len(corr_m.columns)):
            if abs(corr_m.iloc[i,j]) >= 0.90:
                ...
                cr1 = abs(X_tr_sc[c1].corr(y_train))
                cr2 = abs(X_tr_sc[c2].corr(y_train))
