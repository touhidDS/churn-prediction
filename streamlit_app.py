...
def run_pipeline(df, target_column):
    ...
    try:
        if hasattr(best_model, 'feature_importances_'):
            ...
            # Handle list-of-arrays (old SHAP) and 3-D arrays (new SHAP)
            if isinstance(sv, list):
                shap_values = sv[1] if len(sv) > 1 else sv[0]
            elif isinstance(sv, np.ndarray) and sv.ndim == 3:
                shap_values = sv[:, :, 1]   # take class-1 slice → (n_samples, n_features)
            else:
                shap_values = sv
        else:
...
def generate_pdf_report(results_df, best_model_name, best_model,
    ...
    if shap_values is not None:
        _sv_pdf = shap_values if shap_values.ndim == 2 else shap_values[:, :, 1]
        mean_shap = pd.Series(np.abs(_sv_pdf).mean(axis=0),
