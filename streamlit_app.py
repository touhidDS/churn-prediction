...
for key in ['df_raw','df_encoded','results_df','models','best_model_name','sql_table_loaded',
            ...
            'target_column','lime_explainer','lime_series_global']:
...
def run_pipeline(df, target_column):
    ...
    def _aggressive_clean(series):
        cleaned = (series.astype(str)
            ...
            .str.replace(r'[(){}[\]]', '', regex=True)
            .str.replace(r'[^0-9.]', '', regex=True)     # no dash here — already removed above
            .str.strip())
    ...
    for col in cat_cols:
        ...
        if n <= 2:
            ...
            dropped_high_card.append((col, n))
            df = df.drop(columns=[col])
    ...
    try:
        ...
        if isinstance(sv, list):
            sv = sv[1] if len(sv) > 1 else sv[0]   # old SHAP list API
        ...
        shap_exp_2d = shap.Explanation(
            ...
            base_values = (shap_exp_all.base_values[:, 1]
                           if shap_exp_all.base_values.ndim == 2
                           else shap_exp_all.base_values),
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    if shap_exp_2d is not None:
        ...
        try:
            fig_bees, ax_bees = plt.subplots(figsize=(10, max(4, len(features) * 0.4)))
            fig_bees.patch.set_facecolor('white')
            shap.summary_plot(_sv, X_test, show=False)
            plt.title('SHAP Summary — Impact on Churn', fontsize=13, fontweight='bold')
            plt.tight_layout()
            st.pyplot(fig_bees, use_container_width=True); plt.close(fig_bees)
        except Exception:
            pass
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    <div class="page-header">
        ...
        <p>Local Interpretable Model-agnostic Explanations — বুঝুন প্রতিটি কাস্টমারের জন্য মডেল কেন এই সিদ্ধান্ত নিয়েছে।</p>
    </div>
    """, unsafe_allow_html=True)

    # ── LIME vs SHAP comparison info ────────────────────────────────────────
    st.markdown("""
    <div class="insight-card" style="margin-bottom:16px;">
        <b>🔍 LIME কী?</b><br>
        <span style="font-size:0.88rem;color:#94a3b8;">
        LIME (Local Interpretable Model-agnostic Explanations) একটি নির্দিষ্ট কাস্টমারের আশেপাশে
        কৃত্রিম ডেটা তৈরি করে এবং একটি সহজ মডেল দিয়ে বোঝায় — কোন feature এই একজন কাস্টমারের
        প্রেডিকশনে সবচেয়ে বেশি প্রভাব ফেলেছে। SHAP যেখানে গ্লোবাল ও লোকাল দুটোই দেখায়,
        LIME শুধু লোকাল (individual) explanation-এ বিশেষজ্ঞ।
        </span>
    </div>
    """, unsafe_allow_html=True)

    # ── SHAP vs LIME comparison table ───────────────────────────────────────
    st.markdown('<div class="section-title">⚖️ SHAP vs LIME — তুলনা</div>', unsafe_allow_html=True)
    col_s, col_l = st.columns(2)
    with col_s:
        st.markdown("""
        <div class="kpi-card blue" style="padding:18px 20px;">
            <div style="font-size:1.1rem;font-weight:700;color:#3b82f6;margin-bottom:10px;">📊 SHAP</div>
            <div style="font-size:0.83rem;color:#94a3b8;line-height:1.8;">
            ✅ Global + Local উভয় explanation<br>
            ✅ গাণিতিকভাবে নির্ভুল (Shapley values)<br>
            ✅ Tree model-এ অত্যন্ত দ্রুত<br>
            ✅ Feature interaction দেখায়<br>
            ⚠️ জটিল মডেলে ধীর হতে পারে
            </div>
        </div>
        """, unsafe_allow_html=True)
    with col_l:
        st.markdown("""
        <div class="kpi-card green" style="padding:18px 20px;">
            <div style="font-size:1.1rem;font-weight:700;color:#10b981;margin-bottom:10px;">🧪 LIME</div>
            <div style="font-size:0.83rem;color:#94a3b8;line-height:1.8;">
            ✅ যেকোনো model-এ কাজ করে (model-agnostic)<br>
            ✅ Individual prediction-এ বিশেষজ্ঞ<br>
            ✅ সহজে বোধগম্য ব্যাখ্যা<br>
            ✅ প্রতিটি কাস্টমারের জন্য আলাদা<br>
            ⚠️ Random sampling তাই প্রতিবার সামান্য ভিন্ন হতে পারে
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    def build_lime_explainer(_X_train, _feature_names):
        return LimeTabularExplainer(
            training_data  = np.array(_X_train),
            feature_names  = list(_feature_names),
            class_names    = ['Not Churn ✅', 'Churn ⚠️'],
            mode           = 'classification',
            random_state   = 42
        )

    with st.spinner("🔄 LIME Explainer তৈরি হচ্ছে..."):
        lime_explainer = build_lime_explainer(X_train_res, features)

    st.success("✅ LIME Explainer প্রস্তুত!")

    # ── Global LIME — average feature importance ─────────────────────────────
    st.markdown('<div class="section-title">🌍 Global LIME — সামগ্রিক Feature Importance</div>', unsafe_allow_html=True)
    st.markdown("""
    <div class="insight-card amber" style="margin-bottom:12px;">
        <span style="font-size:0.85rem;">
        নিচের চার্টটি Test Set-এর প্রথম <b>50 জন কাস্টমার</b>-এর LIME explanation-এর গড় দেখাচ্ছে।
        এটি বোঝায় — সামগ্রিকভাবে কোন feature চার্ন প্রেডিকশনে সবচেয়ে বেশি প্রভাব ফেলছে।
        </span>
    </div>
    """, unsafe_allow_html=True)

    with st.spinner("🔄 Global LIME importance গণনা হচ্ছে... (কিছুটা সময় লাগবে)"):
        try:
            n_samples = min(50, len(X_test))
            lime_importances = np.zeros(len(features))
            ...
            for i in range(n_samples):
                ...
                # label 1 = Churn class
                label = 1 if 1 in exp.as_map() else list(exp.as_map().keys())[0]
                feat_imp = dict(exp.as_map()[label])
                for feat_idx, imp_val in feat_imp.items():
                    lime_importances[feat_idx] += abs(imp_val)

            lime_importances /= n_samples
            lime_series = pd.Series(lime_importances, index=features).sort_values(ascending=False)

            # Plot global LIME importance
            fig, ax = plt.subplots(figsize=(10, 5))
            fig.patch.set_facecolor('#0d1117'); ax.set_facecolor('#0d1117')
            top_lime = lime_series.head(12)
            bar_colors_lime = ['#10b981' if i < 3 else '#3b82f6' if i < 6 else '#475569'
                               for i in range(len(top_lime)-1, -1, -1)]
            bars = ax.barh(top_lime.index[::-1], top_lime.values[::-1],
                           color=bar_colors_lime, alpha=0.92)
            ax.set_xlabel('Mean |LIME Weight| (গড় প্রভাব)', color='#94a3b8', fontsize=11)
            ax.set_title(f'Global LIME Feature Importance — {best_model_name}',
                         color='#f1f5f9', fontsize=13, fontweight='bold', pad=12)
            ax.tick_params(colors='#94a3b8', labelsize=10)
            for spine in ax.spines.values(): spine.set_color('#1e2733')
            for bar, val in zip(bars, top_lime.values[::-1]):
                ax.text(val + top_lime.values.max()*0.01,
            ...
            # Top 5 LIME feature cards
            st.markdown('<div class="section-title">🌟 LIME-এর শীর্ষ ৫ Feature</div>', unsafe_allow_html=True)
            cols_lime = st.columns(min(5, len(lime_series)))
            for i, (feat, val) in enumerate(lime_series.head(5).items()):
                impact_cls = "green" if i < 2 else "amber" if i < 4 else "blue"
                with cols_lime[i]:
                    st.markdown(f"""
                    <div class="kpi-card {impact_cls}" style="text-align:center;padding:16px 12px;">
                        <div class="kpi-label">#{i+1} LIME Feature</div>
                        <div style="color:#e2e8f0;font-weight:600;font-size:0.85rem;margin:8px 0;word-break:break-word;">{feat}</div>
                        <div style="font-size:0.75rem;color:#475569;margin-top:3px;">weight: {val:.4f}</div>
                    </div>
                    """, unsafe_allow_html=True)

            # Store for later use
            st.session_state['lime_explainer']   = lime_explainer
            st.session_state['lime_series_global'] = lime_series

        except Exception as e:
            st.error(f"❌ Global LIME error: {e}")
            import traceback; st.code(traceback.format_exc())

    st.markdown("<br>", unsafe_allow_html=True)

    # ── Individual LIME explanation ──────────────────────────────────────────
    st.markdown('<div class="section-title">👤 Individual LIME — একজন কাস্টমারের ব্যাখ্যা</div>', unsafe_allow_html=True)

    idx_list     = X_test.index.tolist()
    selected_idx = st.selectbox("একজন কাস্টমার বেছে নিন (Customer ID)", idx_list, key="lime_customer_select")
    row_pos      = idx_list.index(selected_idx)

    customer_row = X_test.loc[selected_idx]
    prob         = best_model.predict_proba(customer_row.values.reshape(1, -1))[0][1]
    ...
    if st.button("🔬 এই কাস্টমারের LIME Explanation দেখুন", type="primary", use_container_width=True):
        with st.spinner("🔄 LIME explanation তৈরি হচ্ছে..."):
            try:
                lime_exp = lime_explainer.explain_instance(
                    data_row     = customer_row.values,
                    predict_fn   = best_model.predict_proba,
                    num_features = min(10, len(features)),
                    top_labels   = 2
                )

                label = 1 if 1 in lime_exp.as_map() else list(lime_exp.as_map().keys())[0]
                lime_feat_imp = lime_exp.as_list(label=label)

                # ── LIME bar chart ───────────────────────────────────────────
                feat_names_lime = [x[0] for x in lime_feat_imp]
                feat_vals_lime  = [x[1] for x in lime_feat_imp]

                fig, ax = plt.subplots(figsize=(10, max(4, len(feat_names_lime) * 0.5)))
                fig.patch.set_facecolor('#0d1117'); ax.set_facecolor('#0d1117')
                bar_colors_ind = ['#ef4444' if v > 0 else '#10b981' for v in feat_vals_lime]
                bars = ax.barh(range(len(feat_names_lime)), feat_vals_lime,
                               color=bar_colors_ind, alpha=0.9, edgecolor='none')
                ax.set_yticks(range(len(feat_names_lime)))
                ax.set_yticklabels(feat_names_lime, color='#94a3b8', fontsize=9.5)
                ax.axvline(0, color='#475569', linewidth=1.2, linestyle='--')
                ax.set_xlabel('LIME Weight  (🔴 positive = চার্ন বাড়ায়  |  🟢 negative = চার্ন কমায়)',
                              color='#94a3b8', fontsize=10)
                ax.set_title(f'LIME Explanation — Customer {selected_idx}  |  Churn Prob: {prob*100:.1f}%',
                             color='#f1f5f9', fontsize=12, fontweight='bold', pad=12)
                ax.tick_params(colors='#94a3b8', labelsize=9)
                for spine in ax.spines.values(): spine.set_color('#1e2733')
                for bar, val in zip(bars, feat_vals_lime):
                    offset = max(abs(v) for v in feat_vals_lime) * 0.02
                    ax.text(val + (offset if val >= 0 else -offset),
                            bar.get_y() + bar.get_height()/2,
                            f'{val:+.4f}', va='center',
                            ha='left' if val >= 0 else 'right',
                            color='#cbd5e1', fontsize=8.5)
                ax.grid(axis='x', color='#1e2733', linewidth=0.5)
                plt.tight_layout()
                st.pyplot(fig, use_container_width=True); plt.close()

                # ── Plain Bangla explanation ─────────────────────────────────
                st.markdown('<div class="section-title">💬 বাংলায় ব্যাখ্যা</div>', unsafe_allow_html=True)

                st.markdown(f"""
                <div class="insight-card {risk_cls}" style="margin-bottom:12px;">
                    <b>কাস্টমার {selected_idx}</b> — এই কাস্টমারের চার্ন সম্ভাবনা <b>{prob*100:.1f}%</b>।
                    মডেল তাকে <b>{risk_label}</b> হিসেবে চিহ্নিত করেছে।
                </div>
                """, unsafe_allow_html=True)

                for feat_name, weight in lime_feat_imp:
                    direction = "🔴 বাড়িয়েছে" if weight > 0 else "🟢 কমিয়েছে"
                    cls       = "red" if weight > 0 else "green"
                    magnitude = "অনেক বেশি" if abs(weight) > 0.1 else "মাঝারিভাবে" if abs(weight) > 0.05 else "সামান্য"
                    st.markdown(f"""
                    <div class="insight-card {cls}" style="margin-bottom:6px;">
                        <b>{feat_name}</b> &nbsp;→&nbsp; চার্ন সম্ভাবনা {magnitude} {direction}
                        &nbsp;<span style="color:#475569;font-size:0.82rem;">(weight: {weight:+.4f})</span>
                    </div>
                    """, unsafe_allow_html=True)

                # ── Prediction probabilities ─────────────────────────────────
                st.markdown('<div class="section-title">📊 Model Prediction Breakdown</div>', unsafe_allow_html=True)
                probs_all = lime_exp.predict_proba
                col_p1, col_p2 = st.columns(2)
                with col_p1:
                    st.markdown(f"""
                    <div class="kpi-card green" style="text-align:center;padding:20px;">
                        <div class="kpi-icon">✅</div>
                        <div class="kpi-value" style="color:#10b981;">{probs_all[0]*100:.1f}%</div>
                        <div class="kpi-label">Not Churn সম্ভাবনা</div>
                    </div>""", unsafe_allow_html=True)
                with col_p2:
                    st.markdown(f"""
                    <div class="kpi-card red" style="text-align:center;padding:20px;">
                        <div class="kpi-icon">⚠️</div>
                        <div class="kpi-value" style="color:#ef4444;">{probs_all[1]*100:.1f}%</div>
                        <div class="kpi-label">Churn সম্ভাবনা</div>
                    </div>""", unsafe_allow_html=True)
    ...
    if st.session_state.get('lime_series_global') is not None and st.session_state.get('shap_values') is not None:
        st.markdown('<div class="section-title">🔄 SHAP vs LIME — Feature Ranking তুলনা</div>', unsafe_allow_html=True)
        st.markdown("""
        <div class="insight-card" style="margin-bottom:12px;">
            <span style="font-size:0.85rem;color:#94a3b8;">
            নিচে SHAP এবং LIME উভয় পদ্ধতির top feature ranking পাশাপাশি দেখানো হয়েছে।
            দুটো পদ্ধতি একই feature কে গুরুত্বপূর্ণ মনে করলে সেটি আরও নির্ভরযোগ্য।
            </span>
        </div>
        """, unsafe_allow_html=True)

        shap_vals = st.session_state['shap_values']
        _sv       = shap_vals if shap_vals.ndim == 2 else shap_vals[:, :, 1]
        shap_series = pd.Series(np.abs(_sv).mean(axis=0), index=features).sort_values(ascending=False)
        lime_series = st.session_state['lime_series_global']

        # Rank comparison
        shap_ranks = {feat: i+1 for i, feat in enumerate(shap_series.index)}
        lime_ranks = {feat: i+1 for i, feat in enumerate(lime_series.index)}
        all_feats  = list(shap_series.head(10).index)

        comparison_data = []
        for feat in all_feats:
            s_rank = shap_ranks.get(feat, '-')
            l_rank = lime_ranks.get(feat, '-')
            agree  = "✅ একমত" if isinstance(s_rank, int) and isinstance(l_rank, int) and abs(s_rank - l_rank) <= 2 else "⚠️ ভিন্নমত"
        ...
        shap_top = shap_series.head(top_n)
        ax1.barh(shap_top.index[::-1], shap_top.values[::-1],
                 color='#3b82f6', alpha=0.85)
        ax1.set_title('SHAP — Top Features', color='#f1f5f9', fontsize=11, fontweight='bold')
        ax1.set_xlabel('Mean |SHAP|', color='#94a3b8', fontsize=9)
        ax1.tick_params(colors='#94a3b8', labelsize=8.5)
        for spine in ax1.spines.values(): spine.set_color('#1e2733')
        ax1.grid(axis='x', color='#1e2733', linewidth=0.5)

        # LIME side
        ax2.set_facecolor('#0d1117')
        lime_top = lime_series.head(top_n)
        ax2.barh(lime_top.index[::-1], lime_top.values[::-1],
                 color='#10b981', alpha=0.85)
        ax2.set_title('LIME — Top Features', color='#f1f5f9', fontsize=11, fontweight='bold')
        ax2.set_xlabel('Mean |LIME Weight|', color='#94a3b8', fontsize=9)
        ax2.tick_params(colors='#94a3b8', labelsize=8.5)
        for spine in ax2.spines.values(): spine.set_color('#1e2733')
        ax2.grid(axis='x', color='#1e2733', linewidth=0.5)

        plt.suptitle('SHAP vs LIME — Feature Importance তুলনা',
    ...
    if shap_values is not None:
        ...
        if shap_exp_2d is not None:
            try:
                import matplotlib
                matplotlib.use('Agg')
                fig_wf, ax_wf = plt.subplots(figsize=(10, max(3.5, len(features) * 0.35)))
                fig_wf.patch.set_facecolor('white')
                shap.plots.waterfall(shap_exp_2d[row_pos], show=False)
                plt.title(f'Why This Customer May Churn — Customer {selected_idx}',
                          fontsize=12, fontweight='bold')
                plt.tight_layout()
                st.pyplot(fig_wf, use_container_width=True); plt.close(fig_wf)
            except Exception:
                # Fallback to horizontal bar chart if waterfall fails
