...
def smart_numeric_convert(df):
    ...
    for col in df.columns:
        # After _df_to_numpy_dtypes, all numeric cols are already float64
        # Skip if already a plain numpy numeric dtype
...
def _safe_to_numpy(series, dtype):
    """
    Convert a pandas Series to a numpy array safely,
    bypassing the Arrow __array__ protocol which crashes on Streamlit Cloud
    when pandas uses the pyarrow backend.
    .to_pylist() extracts Python objects first — always works regardless of backend.
    """
    try:
        return np.array(series.to_pylist(), dtype=dtype)
    except AttributeError:
        # fallback for non-Arrow series
        return np.array(series.tolist(), dtype=dtype)


def _df_to_numpy_dtypes(df):
    """
    Convert an entire DataFrame to plain numpy dtypes.
    Works column-by-column using _safe_to_numpy so Arrow backend never interferes.
    """
    result = {}
    for col in df.columns:
        s = df[col]
        # Try numeric first
        try:
            vals = _safe_to_numpy(s, np.float64)
            # If it looks like integers (no fractional part), keep as float64 still
            # (safer than int — sklearn/SMOTE handles float fine)
            result[col] = pd.array(vals, dtype=np.float64)
        except (ValueError, TypeError):
            # Non-numeric — keep as object
            try:
                result[col] = pd.array(_safe_to_numpy(s, object), dtype=object)
            except Exception:
                result[col] = s.values
...
def run_full_pipeline(file_bytes, file_ext, table_name, target_col):
    ...
    # ── Strip ALL Arrow/nullable dtypes using safe helper ───────
    # _df_to_numpy_dtypes uses .to_pylist() internally — bypasses Arrow __array__
    df = _df_to_numpy_dtypes(df)
    ...
    # Cast y to plain numpy int32 using _safe_to_numpy — never calls Arrow __array__
    y_train = pd.Series(_safe_to_numpy(y_train, np.int32), index=y_train.index, name=y_train.name)
    y_test  = pd.Series(_safe_to_numpy(y_test,  np.int32), index=y_test.index,  name=y_test.name)
    ...
    if len(num_cols) >= 2:
        try:
            # Use pure numpy corrcoef — fully Arrow-immune, index-safe
            X_np  = df_tr[num_cols].to_numpy(dtype=np.float64)
            y_np  = np.array(y_train, dtype=np.float64)
            # corrcoef returns matrix of shape (n_features+1, n_features+1)
            mat   = np.corrcoef(X_np.T, y_np)
            corrs = np.abs(mat[:-1, -1])         # correlation of each feature with target
            top2_idx = np.argsort(corrs)[-2:][::-1]
            top2  = [num_cols[i] for i in top2_idx]
            new_feat = f"{top2[0]}_x_{top2[1]}"
            df_tr[new_feat] = (df_tr[top2[0]].to_numpy(dtype=np.float64) *
                               df_tr[top2[1]].to_numpy(dtype=np.float64))
            df_te[new_feat] = (df_te[top2[0]].to_numpy(dtype=np.float64) *
                               df_te[top2[1]].to_numpy(dtype=np.float64))
        except Exception:
            pass  # skip interaction feature silently if it fails
