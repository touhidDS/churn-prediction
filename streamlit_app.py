...
def run_full_pipeline(file_bytes, file_ext, table_name, target_col):
    ...
    # ── Strip ALL Arrow / nullable dtypes — one definitive pass ─
    # pandas 2.x + pyarrow backend produces ArrowDtype on every column.
    # The only reliable fix is to round-trip the whole DataFrame through
    # numpy-native dtypes BEFORE any processing or splitting happens.
    def _to_numpy_dtypes(frame):
        """Convert every column to a plain numpy dtype — no ArrowDtype, no nullable Int64."""
        out = {}
        for col in frame.columns:
            s = frame[col]
            # Detect Arrow-backed series
            dtype_str = str(s.dtype).lower()
            is_arrow  = (hasattr(s.dtype, "pyarrow_dtype") or
                         "arrow" in dtype_str or
                         dtype_str.startswith("double") or
                         dtype_str.startswith("int") and "[pyarrow]" in dtype_str)
            if is_arrow:
                try:
                    s = s.astype(object)
                except Exception:
                    pass
            # Now convert to the correct numpy dtype
            try:
                out[col] = pd.to_numeric(s, errors='ignore')
            except Exception:
                out[col] = s
        result = pd.DataFrame(out, index=frame.index)
        # Final pass: any remaining non-standard numeric → float64
        for col in result.select_dtypes(include='number').columns:
            if result[col].dtype not in [np.float64, np.float32, np.int64, np.int32, np.int16, np.int8]:
                result[col] = result[col].astype(np.float64)
        return result

    df = _to_numpy_dtypes(df)
    ...
    # Cast y to plain numpy int array — kills any Arrow dtype on the target column
    y_train = pd.Series(np.array(y_train, dtype=np.int32), index=y_train.index, name=y_train.name)
    y_test  = pd.Series(np.array(y_test,  dtype=np.int32), index=y_test.index,  name=y_test.name)
