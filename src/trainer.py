...
def prune_correlated_features(X_train, X_test, y_train, threshold=0.90):
    ...
    to_remove = set()
    y_train = pd.Series(y_train.values).astype('float64').values
    for i in range(len(corr_matrix.columns)):
        for j in range(i + 1, len(corr_matrix.columns)):
            if abs(corr_matrix.iloc[i, j]) >= threshold:
                col1, col2 = corr_matrix.columns[i], corr_matrix.columns[j]
                corr1 = abs(X_train[col1].astype('float64').corr(y_train))
                corr2 = abs(X_train[col2].astype('float64').corr(y_train))
